...
public class Main {
    public static void main(String[] args) {
        // java.time의 클래스들은 생성자 대신 필요에 따라 클래스 메소드로 인스턴스 생성
        ...
        // now 메서드 : 현재의 시간/ 날짜 정보를 가진 해당 클래스의 인스턴스를 반환, 시스템의 시계를 기준
        ...
        LocalTime lunchTimeDetailed = LocalTime.of(12,30,5,3);
        // 초 , 나노 초까지 더할 수 있다.
        ...
        // java.time의 Local  클래스 인스턴스들은 불변
        // 생성자 사용이 금지 -> 인스턴스 생성을 제한 why? 멀티쓰레드 환경
        // of 메소드를 사용하여 , 적절할 때만 생성된 것을 받도록

        // 주어진 차이만큼이나 시간/날짜를 새로 만들어 반환 -> 원본유지
        today.plusDays(1); // 기존 인스턴스는 변하지 않는다.
        LocalDate tomorrow = today.plusDays(1);
        LocalDate yesterday = today.minusDays(1);

        LocalDate[] localDates = {today, tomorrow, yesterday};
        for(LocalDate day : localDates){
            System.out.println(day);
        }

        LocalDate threeMonthsLater = today.plusMonths(3);
        LocalDate tenYearsBefore = today.minusDays(10);
        ...
        // zonedDateTime : 시간대 정보를 추가로 가진다.
        // 컴퓨터가 어느 시간대의 시간을 가지는가
